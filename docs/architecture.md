...
보정 결과는 제품·구현 스킬이 이어받고 다시 정의하지 않습니다. 작은 UI 수정은 변경된 조작·접근성 의미·영향받는 화면을 먼저 검사하고, 통합 위험이나 실패가 있을 때 검증을 넓힙니다. 구현 스킬의 완료 조건에 자동 커밋이나 사용자 미커밋 변경 제거를 포함하지 않습니다. 큰 변경도 가능한 위험 구간부터 리뷰하고 미검토 범위를 명시하며, 분할 권고 때문에 리뷰 자체를 중단하지 않습니다.

설치 검사에는 `--codex /path/to/codex`를 전달해 스킬 노출과 라우팅 검사가 같은 런타임을 사용하도록 합니다. 저장소 검사는 기존 `--static`으로 계정 없이 실행합니다.
