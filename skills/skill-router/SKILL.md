...
- Recommend `$media-image-director` when the user asks for image generation, image editing, posters, thumbnails, visual references, product mockups, social cards, game/app visuals, prompt improvement, or image iteration inside Codex.
...
- Codex image generation: use `$media-image-director` for direct image generation/editing, better prompts, posters, thumbnails, social cards, GitHub/portfolio visuals, app/game UI references, product mockup direction, and image iteration. Keep external provider workflows out of the default route unless the user explicitly asks for them.
