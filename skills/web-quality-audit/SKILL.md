...
description: 'Audit an existing website for performance, Core Web Vitals, accessibility, SEO, and browser best practices. Use for a site-quality or Lighthouse-style audit.'
license: MIT
metadata:
...
## Related retained skills

- Use this skill itself for performance, Core Web Vitals, SEO, and general best-practice findings.
- Use [Accessibility](../accessibility/SKILL.md) when a finding needs a focused WCAG audit or remediation.
- Use `$webapp-testing` or `$playwright` when browser-flow verification is needed.
- Use `$security-and-hardening` when findings touch authentication, secrets, permissions, or untrusted input.
