...
1. Reuse the existing brief; resolve missing business area, users, workflow, pain, or outcome only as needed.
...
6. Keep ownership of the consulting deliverable; consult a specialist only for a concrete unresolved subproblem or required artifact format. Do not start a second routing/intake sequence.
...
- Use-case discovery and prioritization -> score directly using the bundled playbook; add `$risk-assessment` when operational risk analysis is an explicit deliverable.
...
7. Concrete next actions and ownership where needed
...
- If the task affects legal, HR, finance, healthcare, or regulated customer data, inspect the material security boundary and apply security guidance where needed; use operational risk assessment only when requested.
