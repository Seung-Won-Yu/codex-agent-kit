---
name: image-to-code
description: 'Turn a screenshot or explicit image-first visual reference into website code by inspecting or generating section images first. Do not use for routine frontend work, text-only redesigns, or a settled design system.'
---
...
When this skill is explicitly selected inside Codex or another environment that supports image generation plus implementation, use an image-first workflow.
...
Trigger image generation first only when the user provides a visual reference or explicitly requests an image-first design-to-code workflow.

Typical triggers:
- reproduce or adapt a supplied screenshot, mockup, or reference image
- generate section-level visual references before implementation
- explore image-first art direction and then convert the chosen direction to code

Direct-code first is more acceptable only when:
- the task is mostly technical
- the user wants a bug fix
- the request is text-only and does not explicitly ask for image-first work
